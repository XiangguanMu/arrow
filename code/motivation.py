...
from utils.synthetic import simulate_er, simulate_var, compare_graphs
...
def surd_bins_test(X, nbins):
    ...
    # Only test for 1 node
    Y = np.vstack([X[0, nlags[0]:], X[:, :-nlags[0]]])
    Y_gpu = cp.asarray(Y)
    hist_gpu, _gpu = cp.histogramdd(Y_gpu.T, bins=nbins)

    p = cp.asnumpy(hist_gpu)
    p += 1e-19
    p /= p.sum()
    Ntot = p.ndim
    Nvars = Ntot - 1
    Nt = p.shape[0]  # number of bins
    inds = range(1, Ntot)

    H  = it.entropy_nvars(p, (0,) )  # H(Q_j^+)
    Hc = it.cond_entropy(p, (0,), range(1,Ntot) )
    info_leak = Hc/H  # Nomralize as percentage. Hc itself is leak information
    
    p_s = p.sum(axis=(*inds,), keepdims=True)
    p_s_gpu = cp.asarray(p_s)
    p_gpu = cp.asarray(p)

    # memory used for a single node computation
    j_list = list(icmb(inds, 0))
    for j_idx in range(len(j_list)):
        ...
        max_used_mem = max(mempool.used_bytes(), max_used_mem)
...
def bins_cost_test(n_nodes, n_ts, lag_range):
    ...
    for b in tqdm(bins):
        print(f'bin range: {b}')
        time_start = time.perf_counter()
        used_mem = surd_bins_test(data, nbins=b)
        time_end = time.perf_counter()
        time_consumed.append(time_end-time_start)
        mem_consumed.append(used_mem)
    tst_dir = '../results/motivations/bins_cost_test/'
    if not os.path.exists(tst_dir):
        os.makedirs(tst_dir)
    with open(tst_dir+'lists.txt', 'w') as f:
        f.write(f'time consumed: {time_consumed}\n')
        f.write(f'memory consumed: {mem_consumed}\n')
